(function () {
  ...
  function requestOpen(r){return ['draft','confirmed','sent','partial'].indexOf(r.status)>=0;}
  ...
  function purchaseProducts() {
    ...
    return (S.products||[]).filter(function(p){if(p.active===false||!init(p))return false;var sp=primarySupplier(p);if(supplier&&(!sp||sp.id!==supplier))return false;if(q&&lower(p.name+' '+p.category_name+' '+(p.barcode||'')+' '+(sp?sp.name:'')).indexOf(q)<0)return false;return state.purchaseTab==='all'||recommendedBase(p)>0||selectedPackages(p)>0;}).sort(categorySort);
  }
  function renderPurchaseCard(p){
    var sp=primarySupplier(p),rec=recommendedPackages(p),selected=selectedPackages(p),ordered=outstandingFor(p),price=currentSupplierPrice(p),estimate=price==null?null:price*selected;
    return '<div class="card"><div class="row"><div class="grow"><div class="name">'+html(p.name)+'</div><div class="muted">'+html(sp?sp.name:'Поставщик не назначен')+'</div></div><b>'+html(moneySafe(price))+'</b></div><div class="muted" style="margin-top:6px">Сейчас: '+html(parts(qty(p),p))+' • минимум '+html(parts(Number(p.minimum_amount||0),p))+(Number(p.target_amount||0)>0?' • цель '+html(parts(Number(p.target_amount),p)):'')+' • уже заказано '+html(ordered?parts(ordered,p):'0')+'</div><div class="row" style="margin-top:9px"><div class="grow"><b>Рекомендуется: '+rec+' '+packageName(p)+'</b></div><div class="v15counter"><button class="secondary v15Minus" data-key="'+html(productKey(p))+'">−</button><b>'+selected+' '+packageName(p)+'</b><button class="v15Plus" data-key="'+html(productKey(p))+'">+</button></div></div>'+(estimate!=null&&selected>0?'<div class="muted" style="text-align:right;margin-top:5px">≈ '+html(moneySafe(estimate))+'</div>':'')+'</div>';
  }
  ...
  async function renderPurchasesV15() {
    ...
    var header='<div class="toolbar"><button class="secondary v15PTab" data-tab="needed">Нужно заказать</button><button class="secondary v15PTab" data-tab="all">Все товары</button><button class="secondary v15PTab" data-tab="requests">Заявки</button></div>';
