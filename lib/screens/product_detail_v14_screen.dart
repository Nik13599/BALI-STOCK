...
class ProductDetailV14Screen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        final lastDelivery = controller.lastDeliveryFor(current);
        final lastLine = controller.lastDeliveryLineFor(current);
        ...
        return Scaffold(
          ...
          body: ListView(
            ...
            children: [
              _ProductHero(product: current, meta: meta),
              const SizedBox(height: 16),
              _Section(
                title: 'Склад',
                icon: Icons.inventory_2_outlined,
                children: [
                  _ValueRow('Категория', current.categoryName),
                  _ValueRow(
                    'Остаток',
                  ...
                  if (current.stockInitialized) _ValueRow('Всего', formatTotalAmount(current.totalAmount, current.stockUnit)),
                  _ValueRow('Минимум', formatMinimumAmount(current.minimumAmount, current.stockUnit)),
                  if (current.targetAmount > 0) _ValueRow('Цель', formatTotalAmount(current.targetAmount, current.stockUnit)),
                  _ValueRow('Код', current.barcode?.trim().isNotEmpty == true ? current.barcode! : 'Не задан'),
                ],
              ),
              ...
              _Section(
                ...
                children: [
                  _ValueRow('Последняя закупочная цена', formatMoney(current.defaultCost, current.costCurrency), strong: true),
                  _ValueRow('Последняя поставка', lastDelivery == null ? '—' : formatDateTime(lastDelivery.createdAt)),
                  _ValueRow('Поставщик', lastDelivery?.supplierName ?? '—'),
                  if (lastLine?.unitCost != null) _ValueRow('Цена в последней поставке', formatMoney(lastLine!.unitCost, current.costCurrency)),
                ],
              ),
              const SizedBox(height: 12),
              _Section(
                ...
                children: [
                  _ValueRow('Статус', meta.sellByBottle ? 'Включена' : 'Выключена'),
                  _ValueRow('Цена бутылки', meta.sellByBottle ? formatMoney(meta.bottleSalePrice) : '—', strong: true),
                  _ValueRow('Валовая прибыль', formatMoney(economics.bottleGrossProfit)),
                  _ValueRow('Наценка', _percent(economics.bottleMarkupPercent)),
                  _ValueRow('Валовая маржа', _percent(economics.bottleMarginPercent)),
                ],
              ),
              const SizedBox(height: 12),
              _Section(
                title: 'Порционная продажа',
                ...
                children: [
                  ...
                  for (final portion in meta.portions) ...[
                    const Divider(height: 18),
                    _ValueRow('${portion.amount} ${current.stockUnit.symbol}', formatMoney(portion.price), strong: true),
              ...
              _Section(
                ...
                children: [
                  _ValueRow('Себестоимость 1 ${current.stockUnit.symbol}', formatMoney(economics.costPerBaseUnit)),
                  _ValueRow('Стоимость фактического остатка', formatMoney(economics.stockCost), strong: true),
                  _ValueRow('Потенциальная выручка бутылками', formatMoney(economics.potentialBottleRevenue())),
              ...
              if (audit.isNotEmpty) ...[
                ...
                for (final entry in audit.take(12)) _AuditTile(entry: entry),
              ],
            ],
          ),
        );
  ...
  Future<void> _editSales(BuildContext context, Product product, ProductV14Meta meta) async {
    ...
    final result = await showDialog<ProductV14Meta>(
      context: context,
      builder: (_) => _SalesEditorDialog(product: product, initial: meta),
    );
    ...
    try {
      ...
      if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Изменения карточки сохранены')));
    } catch (e) {
      if (context.mounted) showErrorSnack(context, e);
    }
  }
  ...
  Future<void> _pickImage(BuildContext context, Product product) async {
    ...
    final file = await openFile(
      acceptedTypeGroups: const [
        XTypeGroup(label: 'Изображения', extensions: ['jpg', 'jpeg', 'png', 'webp', 'heic', 'heif']),
      ],
    );
    ...
    try {
      ...
      final mime = _imageMime(file.name);
      ...
      if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Фото товара сохранено в общей базе')));
    } catch (e) {
      if (context.mounted) showErrorSnack(context, e);
    }
  }
  ...
  Future<void> _spotStocktake(BuildContext context, Product product) async {
    if (!await _authorize(context) || !context.mounted) return;
    final input = await showDialog<_SpotInput>(context: context, builder: (_) => _SpotStocktakeDialog(product: product));
...
class _ProductHero extends StatelessWidget {
  const _ProductHero({required this.product, required this.meta});
  final Product product;
          ...
          children: [
            Container(
              ...
              child: meta.imageUrl == null
                  ...
                  : Image.network(meta.imageUrl!, fit: BoxFit.contain, errorBuilder: (_, __, ___) => const Icon(Icons.broken_image_outlined, size: 42)),
            ),
...
class _SalesEditorDialog extends StatefulWidget {
  const _SalesEditorDialog({required this.product, required this.initial});
  final Product product;
  final ProductV14Meta initial;

  @override
  State<_SalesEditorDialog> createState() => _SalesEditorDialogState();
}

class _SalesEditorDialogState extends State<_SalesEditorDialog> {
  late bool sellByBottle;
  late bool portionSale;
  late final TextEditingController bottlePrice;
  late List<_PortionControllers> portions;
  ...
  void initState() {
    ...
    portions = widget.initial.portions
        .map((x) => _PortionControllers(amount: '${x.amount}', price: x.price.toStringAsFixed(2).replaceAll('.', ',')))
        .toList();
  }
  ...
  void dispose() {
    bottlePrice.dispose();
    for (final p in portions) p.dispose();
    super.dispose();
  }

  double? _money(String value) => double.tryParse(value.trim().replaceAll(',', '.'));

  void _save() {
    final price = _money(bottlePrice.text);
    if (sellByBottle && (price == null || price < 0)) {
      showErrorSnack(context, 'Укажите цену продажи бутылки');
      return;
    }
    final parsed = <PortionPrice>[];
    for (final p in portions) {
      final amount = int.tryParse(p.amount.text.trim());
      final portionPrice = _money(p.price.text);
      if (amount == null || amount <= 0 || portionPrice == null || portionPrice < 0) {
        showErrorSnack(context, 'Проверьте объём и цену всех порций');
        return;
      }
      parsed.add(PortionPrice(amount: amount, price: portionPrice));
    }
    ...
    Navigator.of(context).pop(widget.initial.copyWith(
          ...
          clearBottleSalePrice: !sellByBottle && price == null,
              ...
              children: [
                    ...
                    child: Row(
                      children: [
                        ...
                        IconButton(
                          ...
                          onPressed: () => setState(() {
                            final item = portions.removeAt(i);
                            item.dispose();
                          }),
                ...
                Align(
                  ...
                  child: TextButton.icon(
                    onPressed: () => setState(() => portions.add(_PortionControllers(amount: '40', price: ''))),
...
class _PortionControllers {
  _PortionControllers({required String amount, required String price})
...
class _SpotStocktakeDialog extends StatefulWidget {
  const _SpotStocktakeDialog({required this.product});
  final Product product;

  @override
  State<_SpotStocktakeDialog> createState() => _SpotStocktakeDialogState();
}

class _SpotStocktakeDialogState extends State<_SpotStocktakeDialog> {
            ...
            child: Column(
              ...
              children: [
                ...
                if (widget.product.stockUnit == StockUnit.piece)
                  ...
                  Row(
                    children: [
                      Expanded(child: TextField(controller: whole, keyboardType: TextInputType.number, decoration: InputDecoration(labelText: '${widget.product.stockUnit.packageLabel}'))),
                      const SizedBox(width: 10),
                      Expanded(child: TextField(controller: extra, keyboardType: TextInputType.number, decoration: InputDecoration(labelText: 'Остаток, ${widget.product.stockUnit.symbol}'))),
                    ],
                  ),
                const SizedBox(height: 10),
                ...
                const InfoBanner(
                  ...
                  text: 'Остаток не переписывается бесследно: будет создана отдельная операция «Точечный переучёт» с было → разница → стало.',
                ),
              ],
            ),
          ),
