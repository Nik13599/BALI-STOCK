...
class _StocktakeV2ScreenState extends State<StocktakeV2Screen> with WidgetsBindingObserver {
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    ...
    } else if (state == AppLifecycleState.paused || state == AppLifecycleState.inactive || state == AppLifecycleState.detached || state == AppLifecycleState.hidden) {
      _pauseClock();
      widget.controller.pauseStocktakeDraft(draft.id, _activeSeconds);
      _noteStore.save(draft.id, comments: _comments, rechecked: _rechecked);
    }
  ...
  Future<String?> _askEmployee() async {
    ...
    final result = await showDialog<String>(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        actions: [
          TextButton(onPressed: () => Navigator.pop(dialogContext), child: const Text('Назад')),
          FilledButton(
            onPressed: () {
              final value = field.text.trim();
              if (value.isEmpty) {
  ...
  Future<_DraftAction> _draftChoice(StocktakeDraft draft) async {
    return await showDialog<_DraftAction>(
          ...
          builder: (dialogContext) => AlertDialog(
            ...
            actions: [
              TextButton(onPressed: () => Navigator.pop(dialogContext, _DraftAction.cancel), child: const Text('Назад')),
              OutlinedButton(onPressed: () => Navigator.pop(dialogContext, _DraftAction.restart), child: const Text('Начать сначала')),
              FilledButton(onPressed: () => Navigator.pop(dialogContext, _DraftAction.resume), child: const Text('Продолжить переучёт')),
            ],
          ),
        ) ??
        _DraftAction.cancel;
    ...
    return await showDialog<bool>(
          ...
          builder: (dialogContext) => AlertDialog(
            ...
            actions: [
              ...
              FilledButton(onPressed: () => Navigator.pop(dialogContext, true), child: const Text('Начать заново')),
            ],
          ),
        ) ??
        false;
  ...
  Future<void> _loadDraft(StocktakeDraft draft) async {
    ...
    for (final line in draft.lines) {
      _entries[line.productId] = _CountEntry(
        ...
        extra: line.stockUnit == StockUnit.piece ? '0' : (line.extraAmount == null ? '' : '${line.extraAmount}'),
      );
    }
  ...
  void _startClock() {
    ...
    _clock = Timer.periodic(const Duration(seconds: 1), (_) {
      ...
      if (_activeSeconds - _lastPersistedSeconds >= 15) {
        ...
        widget.controller.saveStocktakeActiveSeconds(_draft!.id, _activeSeconds);
      }
    });
  }
  ...
  void _changed(SavedStocktakeLine line) {
    ...
    _saveTimers[line.productId] = Timer(const Duration(milliseconds: 250), () async {
      final entry = _entries[line.productId]!;
      await widget.controller.saveStocktakeDraftLine(
        draftId: draft.id,
        productId: line.productId,
        wholePackages: int.tryParse(entry.whole.text),
        extraAmount: line.stockUnit == StockUnit.piece ? 0 : int.tryParse(entry.extra.text),
      );
      await _noteStore.save(draft.id, comments: _comments, rechecked: _rechecked);
      if (mounted) setState(() {});
    });
    setState(() {});
  }
  ...
  void _commentChanged(int productId, String value) {
    ...
    if (draft != null) {
      ...
      _saveTimers[-productId] = Timer(const Duration(milliseconds: 400), () => _noteStore.save(draft.id, comments: _comments, rechecked: _rechecked));
    }
  }

  Product? _productFor(SavedStocktakeLine line) {
    for (final product in widget.controller.products) {
      if (product.id == line.productId || product.name.toLowerCase() == line.productName.toLowerCase()) return product;
    }
    return null;
  }
  ...
  bool _isSuspicious(SavedStocktakeLine line) {
    ...
    if (line.beforeTotal > 0 && actual > line.beforeTotal * 5 && actual - line.beforeTotal >= package * 3) return true;
    return false;
  }
  ...
  Future<bool> _performMandatoryRechecks() async {
    final draft = _draft!;
    final flagged = draft.lines.where((line) => _isSuspicious(line) && !_rechecked.contains(line.productId)).toList(growable: false);
        ...
        if (mounted) {
          await showDialog<void>(
            ...
            builder: (dialogContext) => AlertDialog(
              ...
              content: Text('${line.productName}: первое значение и повторный пересчёт не совпали. В карточку подставлен результат повторного пересчёта. Пересчитайте позицию ещё раз и затем снова завершите переучёт.'),
              actions: [FilledButton(onPressed: () => Navigator.pop(dialogContext), child: const Text('Понятно'))],
            ),
          );
        }
        return false;
    ...
    final result = await showDialog<int>(
      ...
      builder: (dialogContext) => StatefulBuilder(
        builder: (context, setState) {
          final before = formatStockParts(line.beforeTotal, line.packageSize, line.stockUnit);
          final actual = _actualTotal(line)!;
              ...
              child: Column(
                ...
                children: [
                  InfoBanner(
                    ...
                    text: 'Обнаружено существенное расхождение. Расчётно было $before, первый пересчёт дал ${formatStockParts(actual, line.packageSize, line.stockUnit)} (${diff >= 0 ? '+' : '−'}${formatTotalAmount(diff.abs(), line.stockUnit)}). Введите результат повторного физического пересчёта заново.',
                  ),
                  const SizedBox(height: 14),
                  if (line.stockUnit == StockUnit.piece)
                    IntegerField(controller: whole, label: 'Повторно пересчитано, шт.', min: 0)
                  else
                    TwoFields(
                      first: IntegerField(controller: whole, label: line.stockUnit == StockUnit.ml ? 'Бутылок повторно' : 'Упаковок повторно', min: 0),
                      second: IntegerField(controller: extra, label: 'Доп. ${line.stockUnit.symbol}', min: 0),
                    ),
                ],
              ),
            ),
              ...
              FilledButton(
                onPressed: () {
                  ...
                  if (e == null || e < 0 || e >= line.packageSize) {
                    showErrorSnack(dialogContext, 'Дополнительный остаток должен быть от 0 до ${line.packageSize - 1} ${line.stockUnit.symbol}');
                    return;
                  }
        ...
        builder: (context, setState) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Сотрудник: ${_draft!.employeeName}', style: const TextStyle(fontWeight: FontWeight.w900)),
                  const SizedBox(height: 8),
                  Text('Пересчитано: ${_draft!.totalCount} из ${_draft!.totalCount} позиций'),
                  Text('Активное время: ${formatDurationSeconds(_activeSeconds)}'),
                  const SizedBox(height: 12),
                  CheckboxListTile(
                    contentPadding: EdgeInsets.zero,
                    value: confirmed,
                    onChanged: (value) => setState(() => confirmed = value == true),
                    title: const Text('Подтверждаю, что все позиции физически пересчитаны, а введённые значения соответствуют фактическому наличию.'),
                    controlAffinity: ListTileControlAffinity.leading,
                  ),
                  const SizedBox(height: 8),
                  const Text('Подпись сотрудника', style: TextStyle(fontWeight: FontWeight.w800)),
                  const SizedBox(height: 6),
                  AnimatedBuilder(
                    animation: pad,
                    builder: (context, _) => Container(
                      height: 180,
                      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.white24)),
                      clipBehavior: Clip.antiAlias,
                      child: GestureDetector(
                        onPanStart: (details) => pad.start(details.localPosition),
                        onPanUpdate: (details) => pad.add(details.localPosition),
                        onPanEnd: (_) => pad.endStroke(),
                        child: CustomPaint(painter: _SignaturePainter(pad.points), size: Size.infinite),
                      ),
                    ),
                  ),
                  Align(alignment: Alignment.centerRight, child: TextButton.icon(onPressed: pad.clear, icon: const Icon(Icons.clear), label: const Text('Очистить подпись'))),
                ],
              ),
            ),
          ),
          actions: [
            ...
            FilledButton.icon(
              onPressed: () {
                if (!confirmed) {
                  showErrorSnack(dialogContext, 'Подтвердите результаты переучёта');
                  return;
                }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: const Text('Переучёт завершён. Остатки и история обновлены у всех устройств.'),
  ...
  Widget build(BuildContext context) {
    ...
    final draft = _draft;
    if (draft == null) {
      return Scaffold(body: EmptyState(icon: Icons.error_outline, title: 'Переучёт не открыт', message: 'Вернитесь назад и откройте раздел ещё раз.', action: FilledButton(onPressed: widget.onCompleted, child: const Text('Назад'))));
    }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          Center(child: Padding(padding: const EdgeInsets.only(right: 10), child: Text(formatDurationSeconds(_activeSeconds), style: const TextStyle(fontWeight: FontWeight.w800)))),
        ],
      ),
      body: Column(
        children: [
          Container(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Expanded(child: LinearProgressIndicator(value: draft.totalCount == 0 ? 0 : filled / draft.totalCount, minHeight: 9, borderRadius: BorderRadius.circular(10))),
                    const SizedBox(width: 12),
                    Text('$filled / ${draft.totalCount}', style: const TextStyle(fontWeight: FontWeight.w900)),
                  ],
                ),
                    ...
                    children: [
                      for (final group in grouped.entries) ...[
                        Padding(
                          ...
                          child: Text(group.key.toUpperCase(), style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w900, color: Color(0xFF39FF6A), letterSpacing: .5)),
                        ),
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                Expanded(
                  child: Text(
                    complete ? 'Все ${draft.totalCount} позиций заполнены. Перед завершением система проверит крупные расхождения.' : 'Осталось заполнить: ${draft.totalCount - filled}',
                    style: TextStyle(fontWeight: FontWeight.w700, color: complete ? Theme.of(context).colorScheme.primary : null),
                  ),
                ),
                const SizedBox(width: 12),
                FilledButton.icon(
                  onPressed: complete && !_submitting ? _submit : null,
                  icon: _submitting ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.verified_outlined),
                  label: const Padding(padding: EdgeInsets.symmetric(vertical: 10), child: Text('ЗАВЕРШИТЬ ПЕРЕУЧЁТ')),
                ),
              ],
            ),
          ),
  ...
  Widget _lineCard(SavedStocktakeLine line) {
    ...
    final filled = _isFilled(line);
    final actual = _actualTotal(line);
    final diff = actual == null || !line.beforeInitialized ? null : actual - line.beforeTotal;
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            if (line.stockUnit == StockUnit.piece)
              IntegerField(controller: entry.whole, label: 'Фактически, шт.', min: 0, onChanged: (_) => _changed(line))
            else
              TwoFields(
                first: IntegerField(controller: entry.whole, label: line.stockUnit == StockUnit.ml ? 'Целых бутылок' : 'Целых упаковок', min: 0, onChanged: (_) => _changed(line)),
                second: IntegerField(
            ...
            if (diff != null) ...[
              ...
              Text(
                ...
                style: TextStyle(fontWeight: FontWeight.w800, color: diff == 0 ? Theme.of(context).colorScheme.primary : (diff < 0 ? Theme.of(context).colorScheme.error : Theme.of(context).colorScheme.tertiary)),
              ),
            ],
            const SizedBox(height: 10),
            TextFormField(
              ...
              maxLines: 2,
              decoration: const InputDecoration(labelText: 'Комментарий к позиции (необязательно)', hintText: 'Например: бой, две открытые бутылки, найдено на втором баре'),
...
class _SignatureController extends ChangeNotifier {
  ...
  List<List<double>> get serialized => points.whereType<Offset>().map((point) => <double>[point.dx, point.dy]).toList(growable: false);
}
...
class _SignaturePainter extends CustomPainter {
  ...
  @override
  bool shouldRepaint(covariant _SignaturePainter oldDelegate) => oldDelegate.points != points || oldDelegate.points.length != points.length;
}
